...
async function handleCrawl(args: string[]): Promise<number> {
...
async function handleExtract(args: string[]): Promise<number> {
...
async function handleSitemap(args: string[]): Promise<number> {
  const url = args.find(a => !a.startsWith('-'));
  if (!url) {
...
async function handleBench(args: string[]): Promise<number> {
  const url = args.find(a => !a.startsWith('-'));
  if (!url) {
