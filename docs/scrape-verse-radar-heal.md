...
The adapter pins `@brightdata/cli@0.3.2`, verifies the complete command shape, invokes it without a shell, and **never passes `--auto-approve`**. Bright Data should stop at `awaiting_approval`; review `preview_result`, then approve or reject with the official CLI. Approval is intentionally outside RadarHeal.

Execution requires `RADAR_HEAL_TRUSTED_ORIGINS` to contain the exact target origin (for example `https://example.com`) and a POSIX runtime with process-group cleanup. Windows supports plan generation and drift checks; execution fails before starting a process. URL screening rejects IP literals, credentials, and nonstandard HTTPS ports; execution also checks initial DNS with the crawler's network policy. The external CLI and remote collector do not use the crawler's pinned transport: their later DNS answers, redirects, and subresources remain under the operator-reviewed collector's control. This wrapper is not isolation for arbitrary hostile target origins.
