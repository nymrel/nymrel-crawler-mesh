...
export function buildHealCommand(collectorId, prompt, targetUrl) {
...
export async function runHealProposal(command, timeoutMs = DEFAULT_TIMEOUT_MS) {
    ...
    if (!trustedOrigins.includes(target.origin))
        throw new Error('proposal execution requires an exact operator-trusted target origin');
    if (process.platform === 'win32')
        throw new Error('proposal execution requires a POSIX process-group runtime; Windows supports plan generation');
